...
import org.cloudcoder.app.server.persist.txn.GetMostRecentStubTextChange;
...
import org.cloudcoder.app.server.persist.txn.GetProblemIDForProblem;
...
//		this.connectionPool = new C3P0ConnectionPool(config);
	...
	@Override
	public Change getMostRecentStubTextChange(final User user, final int problemId) {
		return databaseRun(new GetMostRecentStubTextChange(problemId, user));
	}
	...
	public Long getProblemIDForProblem(final String problemName) {
		return databaseRun(new GetProblemIDForProblem(problemName));
	}
