...
public class EditCodeServiceImpl  extends RemoteServiceServlet implements EditCodeService {
	...
	public ProblemText doLoadCurrentText(User user, Problem problem) {
	...
	@Override
	public ProblemText logChange(Change[] changeList, long clientSubmitTime)
		...
		String insertTextChange = "";
		String insertLinesChange = "";
		String deleteTextChange = "";
		String deleteLinesChange = "";
		String overallChange = "";
		String fullTextChange = "";
		for (Change change : changeList) {
			long orig = change.getEvent().getTimestamp();
			change.getEvent().setTimestamp(orig + clientServerTimeDelta);
			if(change.getType() == ChangeType.INSERT_TEXT)
			{
				insertTextChange += change.getText();
			}
			else if(change.getType() == ChangeType.INSERT_LINES)
			{
				insertLinesChange += change.getText();
			}
			else if(change.getType() == ChangeType.REMOVE_TEXT)
			{
				deleteTextChange += change.getText();
			}
			else if(change.getType() == ChangeType.REMOVE_LINES)
			{
				deleteLinesChange += change.getText();
			}
			else if(change.getType() == ChangeType.FULL_TEXT)
			{
				fullTextChange += change.getText();
			}
		}

		
		//ProblemText text = doLoadCurrentText(user,problem);
		//ProblemText text = loadCurrentText();
		//overallChange = text.getText();
		...
		//String temp =  Database.getInstance().getMostRecentStubTextChange(user, problem.getProblemId()).getText();
		//System.out.println("***********" + temp);

		SocketIO socket = null;
		ChatSocketCallback callback = null;
		try {
            socket = new SocketIO("http://erebor.lti.cs.cmu.edu:8015/");
            callback = new ChatSocketCallback(user.getId(), problem.getProblemId());
		...
		try
		{
			//ProblemText text1 = doLoadCurrentText(user,problem);
			ProblemText text1 = loadCurrentText();
			overallChange = text1.getText();
			System.out.println("-------->" + overallChange + "-------->");
			socket.emit("sendcode", insertTextChange, insertLinesChange, deleteTextChange, deleteLinesChange, fullTextChange, overallChange);
		}
		catch (Exception e)
		{
		...
		return new ProblemText(text.getText(), callback.happened);
	}
