...
public class DevelopmentPage2 extends CloudCoderPage {
	...
	private static int FLUSH_PENDING_CHANGES_INTERVAL_MS = 30000;
	...
	private class UI extends Composite implements SessionObserver {
		...
		private void flushPendingChanges() {
			if (codeStateManager.getState() == CodeState.EDITABLE_DIRTY) {
				...
				RPC.editCodeService.logChange(changes, System.currentTimeMillis(), new AsyncCallback<ProblemText>() {
					@Override
					public void onFailure(Throwable caught) {
						getSession().add(StatusMessage.error("I am here...."));
						if (caught instanceof CloudCoderAuthenticationException) {
							recoverFromServerSessionTimeout(new Runnable() {
								@Override
								public void run() {
		 ...
		 * @param text
		 */
		protected void doResetProblem(String text) {
